...
//
// A "template" is a saved snapshot of all three build inputs: cloud-config
// `user-data` (with an optional {{HOSTNAME}} placeholder), `meta-data` JSON,
// and an optional `network-config`. Saving/loading all three together means
// picking a template restores the exact combination that produced a working
// ISO before, rather than only the user-data half. `instance-id` inside a
// saved template's meta-data is irrelevant either way, since buildIso()
// always overwrites it with a fresh one - see buildIso's own comment.
//
...
async function saveTemplate({ id, name, userData, metaData = '', networkConfig = '' }) {
  ...
  const record = { id: templateId, name, userData, metaData, networkConfig, updatedAt: new Date().toISOString() };
  if (existingIndex === -1) {
