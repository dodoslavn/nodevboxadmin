...
// Per-VM NAT port-forwarding rules for a `nat`-attached NIC (distinct from
// the host-level "NAT Network" feature elsewhere in this file). Confirmed
// by testing: `showvminfo --machinereadable` reports these as
// `Forwarding(0)=`, `Forwarding(1)=`, etc. - but that index resets to 0 for
// EACH NIC and carries no NIC number in the key itself, so two NAT-attached
// NICs both produce `Forwarding(0)=...`. parseMachineReadable's flat
// `out[key] = value` (used by getVmInfo/parseNics above) would silently let
// the second NIC's rule overwrite the first's - confirmed this actually
// happens. So this does its own raw-text walk of showvminfo's output,
// tracking the most recent `nicN=` line as state (each NIC's block of
// lines, Forwarding(k) included, is contiguous before the next NIC's block
// starts - confirmed via direct output inspection), rather than going
// through parseMachineReadable/getVmInfo at all.
async function getNatRules(uuid) {
  const { stdout } = await runVBoxManage(['showvminfo', uuid, '--machinereadable']);
  const rulesByNic = {};
  let currentNic = null;
  for (const rawLine of stdout.split(/\r?\n/)) {
    const line = rawLine.trim();
    const nicMatch = /^nic(\d+)=/.exec(line);
    if (nicMatch) {
      currentNic = Number(nicMatch[1]);
      continue;
    }
    const fwdMatch = /^Forwarding\(\d+\)="(.*)"$/.exec(line);
    if (fwdMatch && currentNic != null) {
      const [name, protocol, hostIp, hostPort, guestIp, guestPort] = fwdMatch[1].split(',');
      if (!rulesByNic[currentNic]) rulesByNic[currentNic] = [];
      rulesByNic[currentNic].push({ name, protocol, hostIp, hostPort, guestIp, guestPort });
    }
  }
  return rulesByNic;
}

async function addNatRule(uuid, nicIndex, { name, protocol, hostIp, hostPort, guestIp, guestPort }) {
  await runVBoxManage(['modifyvm', uuid, `--nat-pf${nicIndex}`, `${name},${protocol},${hostIp},${hostPort},${guestIp},${guestPort}`]);
  return { ok: true };
}

async function deleteNatRule(uuid, nicIndex, name) {
  await runVBoxManage(['modifyvm', uuid, `--nat-pf${nicIndex}`, 'delete', name]);
  return { ok: true };
}
...
// Each record gets an `existsOnDisk` flag - VirtualBox's registry can go
// stale (a file deleted or moved outside VirtualBox's knowledge stays
// listed until something notices), so this checks the actual filesystem
...
async function listAllMedia() {
  ...
  const media = lists.flat();
  await Promise.all(
    media.map(async (m) => {
      m.existsOnDisk = m.Location ? await fsp.access(m.Location).then(() => true, () => false) : false;
    })
  );
  return media;
}

// De-registers a medium from VirtualBox without touching the file on disk -
// closemedium without --delete (confirmed in registerMedium's comment
// above). Used by the Disks page's "Remove" button, which should only drop
// VirtualBox's own record, not destroy the host file.
async function unregisterMedium(kind, medium) {
  await runVBoxManage(['closemedium', kind, medium]);
  return { ok: true };
}
